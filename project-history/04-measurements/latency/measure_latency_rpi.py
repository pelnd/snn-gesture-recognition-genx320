"""
Pure inference-latency benchmark for the fine-tuned GenX320 SNN -- PI (CPU).
...
def load_checkpoint(net, path, device):
    # kept the same dict-or-raw detection as the other machines' scripts,
    # even though this one's confirmed to be dict-wrapped like
    # checkpoint_latest.pth -- harmless, and keeps all three scripts
    # identical in behavior if that ever changes.
